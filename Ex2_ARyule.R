...
#3. Apply Model
btc_ar <- ar.yw(data_train, order.max = 1, aic = FALSE)
...
btc_ar <- ar.yw(data_train, order.max = 2, aic = FALSE)
btc_pred <- predict(object = btc_ar, n.ahead = 94)
lines(x = xvals, btc_pred$pred, ylim=range(data[,2]), xlim=range(1,376), xlab = "", ylab = "", type = "l", col = "green")

btc_ar <- ar.yw(data_train, order.max = 5, aic = FALSE)
btc_pred <- predict(object = btc_ar, n.ahead = 94)
lines(x = xvals, btc_pred$pred, ylim=range(data[,2]), xlim=range(1,376), xlab = "", ylab = "", type = "l", col = "green")

btc_ar <- ar.yw(data_train, order.max = 50, aic = FALSE)
btc_pred <- predict(object = btc_ar, n.ahead = 94)
lines(x = xvals, btc_pred$pred, ylim=range(data[,2]), xlim=range(1,376), xlab = "", ylab = "", type = "l", col = "blue")

btc_ar <- ar.yw(data_train, order.max = 250, aic = FALSE)
btc_pred <- predict(object = btc_ar, n.ahead = 94)
